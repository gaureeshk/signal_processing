...
\begin{multline}
	...
	=1-\frac{z^{-1}}{2}+\frac{5z^{-2}}{4}-\frac{5z^{-3}}{8}+\frac{5z^{-4}}{16}\\+\frac{\frac{5z^{-3}}{16}}{1+\frac{z^{-1}}{2}}\\
\end{multline}
\begin{multline}
...
\begin{align}
	...
	h(0)=1,h(1)=-\frac{1}{2},h(2)=\frac{5}{4},h(3)=-\frac{5}{8},h(4)=
\end{align}
...
\begin{figure}[!ht]
	...
	\label{fig:hn}
\end{figure}
As we can see from the graph, h(n) is bounded in [-0.625,1.25] and it converges to 0 because $\brak{\frac{-1}{2}}^n$ and $\brak{\frac{-1}{2}}^{n-2}$ both converge to 0 as n approaches $\infty$\\
%
...
\solution 
\begin{align}
	\sum_{n=-\infty}^{\infty}h(n)&=\sum_{n=0}^{\infty}\brak{\frac{-1}{2}}^n+\sum_{n=2}^{\infty}\brak{\frac{-1}{2}}^{n}\\
%	&=\frac{1}{\frac{3}{2}}+\frac{\frac{1}{4}}{\frac{3}{2}}\\
	&=\frac{5}{6}<\infty
\end{align}
...
\begin{equation}
	...
	\label{conv}
\end{equation}
\solution Put $k'=n-k$ in \eqref{conv}
\begin{align}
	\sum_{k'=\infty}^{-\infty}x(k')h(n-k')&=\sum_{k'=-\infty}^{\infty}x(k')h(n-k')\\
	&=y(n)  
\end{align} 
%(using \eqref{eq:convolution})
\end{enumerate}
%
\section{DFT and FFT}
\begin{enumerate}[label=\thesection.\arabic*]
